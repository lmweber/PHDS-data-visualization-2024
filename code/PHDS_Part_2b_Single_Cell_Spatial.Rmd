...
In this session we will use example datasets from single-cell RNA sequencing and spatial transcriptomics to demonstrate additional types of visualizations including reduced dimension plots and heatmaps.

We will use R packages from [Bioconductor](https://bioconductor.org/) for some of the data processing and visualizations.
...
We load an example single-cell dataset from the `TENxPBMCData` package. This package contains single-cell RNA sequencing datasets from peripheral blood mononuclear cells (PBMCs) measured with 10x Genomics platforms and saved in `SingleCellExperiment` format.
...
We run some steps from a short analysis workflow on this dataset. We skip several analysis steps here (in particular quality control), since we are mainly interested in using this dataset for visualization purposes.
...
First, we obtain the top marker genes for cluster 5 with some additional code continuing from the workflow above.
...
An alternative package to create heatmaps is [ComplexHeatmap](https://bioconductor.org/packages/ComplexHeatmap), which provides powerful and detailed options for customized formatting, and includes extensive documentation and tutorials.


# Violin plots

Alternatively, we can also use violin plots to visualize the top marker genes per cluster. Below, we show an example using the `scater` package. This shows the same information as the heatmap above, represented using a different type of visualization. Which is more effective?
...
First, we create a data frame containing the data for each spatial measurement location (spot) stored in `colData(spe)`, `spatialCoords(spe)`, and `reducedDim(spe)`.
