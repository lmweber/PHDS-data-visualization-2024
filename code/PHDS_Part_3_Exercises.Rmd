...
Note that there are some duplicated entries in `places`, which we need to remove before we join the data frames. The duplicated rows can be identified by combining the `city`, `state`, and `country_code` columns, as shown below.
...
We also convert the date and time format by using functions from the `lubridate` package (part of `tidyverse`) to split the values into separate columns, which may be more easily accessible for some types of plots.

```{r}
# convert date and time
ufo_sightings_latlong <- ufo_sightings_latlong |> 
  mutate(year = year(reported_date_time_utc), 
         ...
         second = second(reported_date_time_utc))
