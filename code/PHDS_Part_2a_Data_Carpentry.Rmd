...
Note the syntax used for the `ggplot()` function. The first argument (`data`) provides the input data frame, and the second argument (`mapping = aes()`) specifies the mapping of data columns to plot aesthetics (in this case the `x` and `y` axes), followed by the `+` operator to provide additional functions, and the `geom_point()` function to specify that we want to plot the data as points.
...
We can also specify a different `geom` to plot the data in a different way.

This demonstrates how easily ggplot allows us to change the type of plot, once we have provided the data and specified the mapping of data to plot aesthetics.
...
We can also include color. However, in this case note that we have some redundancy between the facetting and the colors. Depending on the dataset, this type of redundancy may either help the reader or overcomplicate things.
